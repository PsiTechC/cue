...
//         messageIndex++;
//         setTimeout(displayNextMessage, 3000); // Quicker updates
//       }
//     };
//     displayNextMessage();
  
//     try {
//       const totalChunksResponse = await axios.get(`${PYTHON_API_BASE}/get-total-chunks`);
//       setTotalChunks(totalChunksResponse.data.totalChunks);
//     } catch (error) {
...
//       try {
//         const progressResponse = await axios.get(`${PYTHON_API_BASE}/get-progress`);
//         const { processedChunks } = progressResponse.data;
...
//       await shortenUrls(response.data.songs);
...
//   const shortenUrls = async (songs) => {
//     const newShortenedUrls = {};

//     for (const song of songs) {
//       if (song.shazam_link !== 'N/A') {
//         try {
//           const response = await axios.post('${PYTHON_API_BASE}/shorten-url', {
//             url: song.shazam_link,
//             custom: song.title
//           });
//           newShortenedUrls[song.title] = response.data;
//         } catch (error) {
//           console.error('Error shortening URL:', error);
//           newShortenedUrls[song.title] = 'Error shortening URL';
//         }
//       }
//     }
...
//                   <a href={shortenedUrls[song.title]} target="_blank" rel="noopener noreferrer">{shortenedUrls[song.title]}</a>
//                 </td>
//                 <td className="border px-4 py-2">{fileName}</td>
//                 <td className="border px-4 py-2">{song.videoDuration}</td>
//                 <td className="border px-4 py-2">{new Date(song.start_time * 1000).toISOString().substr(11, 8)}</td>
//                 <td className="border px-4 py-2">{new Date(song.end_time * 1000).toISOString().substr(11, 8)}</td>
...
//           className={`py-2 px-4 rounded-md transition-all transform ${
//             disableButtons ? 'bg-[#3d3d3d] text-white cursor-not-allowed' : 'bg-[#28603D] hover:bg-[#417155] hover:scale-102'
//           }`}
...
//                     <div id="processingMessage" className="text-center text-lg mt-4">
//                       Audio processing<span className="dot"></span>
...
//       {processingMessageVisible && (
//         <div id="loadingContainer" className="loading-container mt-5 opacity-100">
...
//             <div className="progress-bar-fill bg-[#417155] h-6 rounded-lg" style={{ width: `${progress}%`, transition: 'width 1s ease' }}></div>
//           </div>
//         </div>
//       )}
...
//               className="bg-[#28603D] hover:bg-[#417155] text-white py-2 px-4 rounded-md transition-all transform"
//             >
//               Download CSV
//             </CSVLink>
//             <button
...
//   // Save form data and detected songs to localStorage whenever they change
//   useEffect(() => {
//     if (detectedSongs.length > 0) {
//       localStorage.setItem('detectedSongs', JSON.stringify(detectedSongs));
//     }
...
//         // Hide loader and update UI once table is ready
//         setAllLinksShortened(true);
//         setShowTable(true);
//         setShowButtons(true);
//         setProcessingMessageVisible(false);
//         setDisableButtons(false);
//         clearInterval(progressInterval); // Ensure interval stops when processing completes
//       } catch (error) {
//         console.error("Error processing audio:", error);
//         setDisableButtons(false);
//         setProcessingMessageVisible(false);
...
const CueSheetGenerator = () => {
  ...
  const [alertType, setAlertType] = useState('');
  const [alertVisible, setAlertVisible] = useState(false); 
  const [isUploading, setIsUploading] = useState(false); // For the loader
...
//   // Update the state when the file is uploaded
//   const handleFileUpload = async () => {
//     const fileInput = document.getElementById('file').files[0];
//     if (!fileInput) {
//       setAlertMessage('Please select an audio file before uploading.');
...
//       const { fileName, userId, videoDuration } = response.data; // Get videoDuration from response
//       setFileName(fileName);
//       setUserId(userId);
//       setVideoDuration(videoDuration); // Set video duration in state
//       document.getElementById('programName').value = fileName;
//       setAlertMessage('File uploaded successfully');
//       setAlertType('success');
//       setAlertVisible(true);
...
//               <th className="border px-4 py-2 bg-gray-200">Program Name</th>
//               <th className="border px-4 py-2 bg-gray-200">Episode Number</th>
//               <th className="border px-8 py-2 bg-gray-200">On-Air Date</th>
//               <th className="border px-4 py-2 bg-gray-200">Track Title</th>
//               <th className="border px-4 py-2 bg-gray-200">Artists</th>
//               <th className="border px-4 py-2 bg-gray-200">Copyright Link</th>
//               <th className="border px-4 py-2 bg-gray-200">Video File Name</th>
//               <th className="border px-4 py-2 bg-gray-200">Video Duration</th>
//               <th className="border px-4 py-2 bg-gray-200">TC In</th>
...
//       <div className="modal-body py-5">
//         <form id="metadataForm" encType="multipart/form-data">
//           <label className="block mb-2" htmlFor="tvChannel">
//             TV Channel:
//           </label>
//           <input
...
//           <label className="block mb-2" htmlFor="programName">
//             Program Name:
//           </label>
//           <input
//             className="w-full mb-4 p-2 border rounded-md"
...
//           <label className="block mb-2" htmlFor="episodeNumber">
//             Episode Number:
//           </label>
//           <input
//             className="w-full mb-4 p-2 border rounded-md"
...
//           <label className="block mb-2" htmlFor="onAirDate">
//             On-Air Date:
//           </label>
//           <input
//             className="w-full mb-4 p-2 border rounded-md"
...
//           <label className="block mb-2" htmlFor="movieAlbum">
//             Movie/Album Name:
//           </label>
//           <input
//             className="w-full mb-4 p-2 border rounded-md"
...
//                 setProcessingMessageVisible(false); // Hide loader after upload
//               }}
//             >
//               Upload File
//             </button>
