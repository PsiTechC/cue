...
//   // Log the token from headers
//   const token = req.headers.authorization;

//   try {
//     const workspaces = await Project.find(); // Fetch all workspaces from the database
//     res.json(workspaces); // Send back the workspaces
//   } catch (err) {
...
//     const deletedWorkspace = await Project.findByIdAndDelete(id);
...
//   try {
//     // Find and update the workspace name
...
// // Route to add a subfolder to an existing folder
// router.post('/folder/:folderId/add-subfolder', authenticateToken, async (req, res) => {
...
//     // Find the parent folder and add the subfolder to its subfolders array
//     const parentFolder = await Folder.findById(folderId);
//     if (!parentFolder) {
//       console.log(`Parent folder not found: ${folderId}`);
//       return res.status(404).json({ message: 'Parent folder not found' });
//     }

//     parentFolder.subfolders.push(newSubfolder._id);
//     await parentFolder.save();

//     console.log(`Subfolder created successfully:`, newSubfolder);
//     res.status(201).json(newSubfolder); // Send back the newly created subfolder
//   } catch (err) {
